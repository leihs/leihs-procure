...
import { default as RequestStateBadge, ProcureStateBadge } from './StateBadges'
              ...
              <Div cls={`form-compact h-100 p-3 bg-light mh-${BIG}-100vh`}>
                ...
                <Div cls={cx(`mt-2 d-${BIG}-block`, { 'd-none': !isOpen })}>
                  <F>
                    ...
                    <Let field={formPropsFor('procureState')}>
                      {({ field }) => (
                        <FormGroup
                          label={t('dashboard.filter_titles.procure_status')}
                        >
                          {available.procureState &&
                            available.procureState.map(({ value, label }) => (
                              <F key={value}>
                                <Div
                                  cls={cx(
                                    'custom-control custom-checkbox',
