...
(defn delete-unused
  ...
  (jdbc/execute! tx (-> (sql/select :%count.*)
                        ...
                        sql-format))
  (jdbc/execute! tx (-> (sql/delete-from :procurement_organizations :po)
                        (sql/where :and [:<> :po.parent_id nil]
                                   [:not [:exists (-> (sql/select true)
                                                      (sql/from [:procurement_requesters_organizations :pro])
                                                      (sql/where [:= :pro.organization_id :po.id]))]]
                                   [:not [:exists (-> (sql/select true)
                                                      ...
                                                      (sql/where [:= :pr.organization_id :po.id]))]])
                        sql-format))
  (jdbc/execute! tx (-> (sql/delete-from :procurement_organizations :po1)
