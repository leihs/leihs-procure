  ...
  context 'some admins, a bunch of users and a bunch of groups exist' do
    ...
    context 'an admin via the UI' do
      ...
      scenario 'edits a group ' do
        visit '/admin/'
        click_on 'Groups'
        group = @groups.filter{|g| g[:protected] == false }.sample # pick a random group
    ...
    context "some inventory-pool's lending-manager " do
      ...
      context 'via the UI' do
        ...
        scenario 'can not edit a protected group' do
          visit '/admin/'
          click_on 'Groups'
          group = @groups.filter{|g| g[:protected] == true }.sample # pick a random group
          expect(group).to be
          fill_in 'term', with: group.name
          click_on group.name
      ...
      context 'via the API' do
        ...
        scenario 'changing the protected field of an unprotected group is forbidden ' do
          group = @groups.filter{|g| g[:protected] == false }.sample # pick a random group
          resp = http_client.patch "/admin/groups/#{group[:id]}", {protected: true }.to_json
