...
RUN jar uf target/leihs-admin.jar resources/built-info.yml
...
COPY --from=builder ${WORKDIR}/target/leihs-admin.jar target/
...
# run
EXPOSE ${HTTP_PORT}
ENTRYPOINT [ "java", "-jar", "target/leihs-admin.jar", "run" ]