(ns leihs.procurement.auth.core
  (:refer-clojure :exclude [str keyword])
  (:require [leihs.procurement.utils.core :refer [keyword str presence]])
  (:require [leihs.procurement.constants :refer [USER_SESSION_COOKIE_NAME]]
...
(defn user-sign-in-base-query
  [email]
  (-> (sql/select :users.id
                  ; :is_admin
                  :sign_in_enabled :firstname
                  :lastname :email)
      (sql/from :users)
      (sql/merge-where [:= (sql/call :lower :users.email)
                        (sql/call :lower email)])
      (sql/merge-where [:= :users.sign_in_enabled true])))

(defn pw-matches-clause
  [pw]
  (sql/call := :users.pw_hash (sql/call :crypt pw :users.pw_hash)))

(defn password-sign-in-query
  [email password secret]
  (-> (user-sign-in-base-query email)
      (sql/merge-join :settings [:= :settings.id 0])
      (sql/merge-where
        [:or (pw-matches-clause password)
         [:and [:= :settings.accept_server_secret_as_universal_password true]
          [:= password secret]]])
      (sql/merge-where [:= :users.password_sign_in_enabled true])
      sql/format))

(defn password-sign-in
  ([{{email :email, password :password, url :url} :form-params,
     tx :tx,
     sba :secret-ba}]
  ...
  ([email password url secret tx]
   (if-let [user (->> (password-sign-in-query email password secret)
                      (jdbc/query tx)
                      first)]
     (session/create-user-session user
                                  secret
                                  (redirect (or (-> url
                                                    presence)
                                                (path :admin))
                                            :see-other)
                                  tx)
     (redirect (path :admin {} {:sign-in-warning true}) :see-other))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defn shib-params->user-params
  [m]
...
(defn sign-out
  ...
  (-> (redirect (or (-> request
                ...
                :see-other)
      (assoc-in
        [:cookies (str USER_SESSION_COOKIE_NAME)]
        {:value "", :http-only true, :max-age -1, :path "/", :secure false})
      (assoc-in
        [:cookies (or (shibbsession-cookie-name request) "bogus")]
        {:value "", :http-only true, :max-age -1, :path "/", :secure false})))
...
(defn get-auth
  ...
  (when (= :json
           (-> request
               :accept
               :mime))
    (when-let [auth-ent (:authenticated-entity request)] {:body auth-ent})))
