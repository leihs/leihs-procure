...
(defn create-requester-organization [tx data]
...
(defn update-requesters-organizations [context args value]
  (let [tx (-> context :request :tx)]
    ...
    (let [req-orgs (jdbc/query tx (sql/format requesters-organizations-base-query))]
      (->> req-orgs
           (map
             #(conj %
                    {:organization (->> %
                                        :organization_id 
                                        (organization/get-organization-by-id tx))}))
           (map
             #(conj %
                    {:department (->> %
                                      :organization
