...
(ns leihs.admin.utils.url.http
  (:require
    [leihs.admin.utils.url.shared :refer [host-port-dissect path-dissect auth-dissect parse-int]]
...
(defn parse-base-url [url]
  (as-> url params
    ...
    (->> params
         (map (fn [[k v]]
                (cond
                  (and (= k :port) (string? v)) [k (parse-int v)]
                  (and (= k :query)
