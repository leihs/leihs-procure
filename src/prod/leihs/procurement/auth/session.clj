(ns leihs.procurement.auth.session
  (:require [cider-ci.open-session.encryptor :as encryptor :refer
             [decrypt encrypt]]
...
(defn delete-session-cookie
  [response]
  (assoc-in response
    [:cookies (str USER_SESSION_COOKIE_NAME)]
    {:value "", :path "/", :max-age 0}))

(defn session-error-page
  [exception request]
  (-> {:status 401,
       :body (str "Session authentication error: " (.getMessage exception))}
      delete-session-cookie))

(defn user-with-valid-session-query
  [token]
  (-> (sql/select [:users.id :user_id]
      ...
      (sql/merge-where
        (sql/call :=
                  ...
                  (sql/call :encode (sql/call :digest token "sha256") "hex")))
      (sql/merge-where
        (sql/raw
...
(defn session-cookie-value
  [request]
  (when-let [cookie (-> request
                        :cookies
                        (get USER_SESSION_COOKIE_NAME nil)
                        :value)]
    (decrypt (-> request
                 :secret-ba
                 String.)
